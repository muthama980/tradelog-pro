...
const STRATEGIES = ['breakout','trend-follow','mean-reversion','scalp','swing','news','support-resistance','other'];
const EMOTIONS   = ['calm','confident','fomo','revenge','fear','greed','impatient','disciplined'];
          ...
          <div className="card rounded-xl w-full max-w-2xl p-8 my-8 relative">
            ...
            <form onSubmit={submit} className="space-y-5">
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <Field label="Market">
                  <select value={form.market} onChange={(e) => setForm({...form, market: e.target.value})} className="form-input capitalize">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <Field label="Direction">
                  <select value={form.direction} onChange={(e) => setForm({...form, direction: e.target.value})} className="form-input">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <Field label="Strategy">
                  <select value={form.strategy} onChange={(e) => setForm({...form, strategy: e.target.value})} className="form-input">
                    <option value="">— select —</option>
                    {STRATEGIES.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </Field>
                <Field label="Emotion">
                  <select value={form.emotion} onChange={(e) => setForm({...form, emotion: e.target.value})} className="form-input">
                    <option value="">— select —</option>
                    {EMOTIONS.map(e_ => <option key={e_} value={e_}>{e_}</option>)}
                  </select>
                </Field>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={submitting} className="btn-primary flex-1 justify-center">
                  {submitting ? <Loader2 className="animate-spin" size={15} /> : editingId ? 'Save Changes' : 'Log Trade'}
                </button>
              </div>
            </form>
          </div>
